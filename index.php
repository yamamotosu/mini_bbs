...
if(!empty($_POST)) {
  if($_POST['message'] !== '') {
    ...
    member_id=?, message=?, reply_message_id=?, 
    created=NOW()');
  ...
  <div id="content">
    ...
    <form action="" method="post">
      <dl>
        ...
        <dd>
          <textarea name="message" cols="50" rows="5"><?php print(htmlspecialchars
          ($message, ENT_QUOTES)); ?></textarea>
          <input type="hidden" name="reply_post_id" value="<?php print(htmlspecialchars($_REQUEST
          ['res'],ENT_QUOTES)); ?>" />
        </dd>
      </dl>
      <div>
...
<?php foreach ($posts as $post): ?>
    ...
    ENT_QUOTES));  ?>" width="48" height="48" alt="
    <?php print(htmlspecialchars($post['name'], ENT_QUOTES)); ?>" />
    <p><?php print(htmlspecialchars($post['message'], ENT_QUOTES)
    );  ?><span class="name">（<?php print(htmlspecialchars($post
    ['name'], ENT_QUOTES)
  );  ?>）</span>[<a href="index.php?res=<?php print
